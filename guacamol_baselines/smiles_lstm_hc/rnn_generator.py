...
class SmilesRnnMoleculeGenerator:
    ...
    def __init__(
        self, model: SmilesRnn, max_len: int, device: str, name_benchmark=None
    ) -> None:
        """
        Args:
    ...
    def optimise(
        ...
        for epoch in range(1, 1 + n_epochs):
            ...
            raws_to_save = [
                {"benchmark": self.name_benchmark, "epoch": epoch, "smiles": smiles}
                for smiles in canonicalized_samples
            ]
            ...
            print("going to score payload ", len(payload))
    ...
    def pretrain_on_initial_population(
        self, scoring_function: ScoringFunction, start_population, pretrain_epochs
    ) -> List[OptResult]:
        """
        Takes an objective and tries to optimise it
